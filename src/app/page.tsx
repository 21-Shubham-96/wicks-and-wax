...
import { Heart, ShoppingCart, Trash, Plus, Minus } from 'lucide-react';
...
export default function Home() {
  ...
  const toggleCart = (id: number) => {
    setCart((prev) => {
      ...
      return item ? prev.filter((i) => i.id !== id) : [...prev, { id, qty: 1 }];
    });
  };
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-start p-6 bg-[#fffaf4]">
      ...
      <div className="w-full flex justify-end gap-4 mb-4">
        <div className="relative cursor-pointer" onClick={() => setViewWishlist(!viewWishlist)}>
        ...
        <div className="relative cursor-pointer" onClick={() => setViewCart(!viewCart)}>
      ...
      {/* Wishlist Modal */}
      {viewWishlist && (
        <div className="w-full max-w-xl bg-white border border-pink-200 rounded-lg p-4 mb-6 shadow-md">
          <h2 className="text-lg font-semibold text-pink-700 mb-2">Your Wishlist</h2>
          {wishlist.length === 0 ? (
            ...
            wishlist.map((id) => (
              <div key={id} className="flex justify-between items-center py-2 border-b">
                <span>{getCandleById(id)?.name}</span>
                <button onClick={() => toggleWishlist(id)} className="text-pink-600">
                  Remove
                </button>
              </div>
            ))
      ...
      {viewCart && (
        <div className="w-full max-w-xl bg-white border border-pink-200 rounded-lg p-4 mb-6 shadow-md">
          <h2 className="text-lg font-semibold text-pink-700 mb-2">Your Cart</h2>
          {cart.length === 0 ? (
            ...
            cart.map(({ id, qty }) => (
              <div key={id} className="flex justify-between items-center py-2 border-b">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => toggleCart(id)} className="text-red-500">
                    <Trash size={16} />
                  </button>
                </div>
              </div>
            ...
            <div className="flex justify-between items-center mt-4">
              <button
                ...
                className={`px-3 py-1 text-sm rounded-full transition ${
                  cart.some((item) => item.id === candle.id)
                    ? 'bg-pink-600 text-white'
                    : 'border border-pink-300 text-pink-600 hover:bg-pink-100'
                }`}
              >
                {cart.some((item) => item.id === candle.id) ? 'Remove from Cart' : 'Add to Cart'}
              </button>
              <button
